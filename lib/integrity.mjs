...
export const TEST_FILE_RE = /(^|[\\/])(__tests__|tests?|spec)[\\/]|\.(test|spec)\.[cm]?[jt]sx?$|_test\.(py|go|rb)$|(^|[\\/])test_[^\\/]*\.py$|Tests?\.(java|kt|cs)$/i;
...
const ASSERT_RE = /\bexpect\(|\bassert(Equal|True|False|Raises|In|Is|That|\.|\s|\()|\.should\b|\bt\.(Error|Fatal)f?\(|\bcheck\(/;
