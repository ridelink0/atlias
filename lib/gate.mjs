...
// (unused here). deps.run stands in for the process runner, so a test can make
// one git or parser call time out and prove the gate still speaks once.
export function stop(payload, _host, deps = {}) {
  const cfg = config();
  ...
  if (!files.length) {
    ...
    const claims = integrity.check({ cwd, turn, last, files: [], flags, cfg: { stubs: false, weakenedTests: false, unwired: false } });
  ...
  if (v.integrity !== false) {
    for (const f of integrity.check({ cwd, turn, last, files, flags, cfg: integrityCfg, incomplete, deps })) {
      raised.push(f.flag);
