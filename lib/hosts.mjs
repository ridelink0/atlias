...
import { ROOT, VERSION, CODEX_DIR, GEMINI_DIR, CLAUDE_DIR, START_MARK, END_MARK, readText, writeText, readJson, writeJson, exists, run, findPython, claudeMemoryDir, writeLauncher, writeHooksLauncher, isVersionedPath, LAUNCHER, installedRoots, ufsCopies, ufsInstallPlan } from './core.mjs';
...
export function mergeToml(text) {
  ...
  const block = ['', '[mcp_servers.atlias]', "command = 'node'", `args = [${tomlString(serverPathForConfig())}]`, 'startup_timeout_sec = 30', ''];
...
export function doctor(cwd = process.cwd()) {
  ...
  const ufsOn = ufsCopies().filter((copy) => copy.loaded);
