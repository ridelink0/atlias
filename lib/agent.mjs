...
// ---------- engines ----------
function cli(cmd, args, input, cwd) {
  const exe = process.platform === 'win32' ? `${cmd}.cmd` : cmd;
  let r = spawnSync(exe, args, { input, encoding: 'utf8', cwd, timeout: 20 * 60 * 1000, windowsHide: true, maxBuffer: 16 * 1024 * 1024 });
  if (r.error && r.error.code === 'ENOENT') r = spawnSync(cmd, args, { input, encoding: 'utf8', cwd, timeout: 20 * 60 * 1000, windowsHide: true, maxBuffer: 16 * 1024 * 1024 });
  return r;
}
export function detectEngines() {
  const ok = (cmd) => cli(cmd, ['--version'], '', process.cwd()).status === 0;
...
export function ollamaAlive(url) {
  try {
    ...
    const probe = `const http=require('node:http');const q=http.get(${JSON.stringify(u.href)},{timeout:1500},(r)=>process.exit(r.statusCode===200?0:1));q.on('error',()=>process.exit(1));q.on('timeout',()=>{q.destroy();process.exit(1)});`;
    return spawnSync(process.execPath, ['-e', probe], { timeout: 5000, windowsHide: true }).status === 0;
  } catch { return false; }
}
export function claudeTurn(state, prompt) {
  const base = ['-p', '--output-format', 'text'];
  let r = cli('claude', [...base, ...(state.started ? ['--resume', state.sid] : ['--session-id', state.sid])], prompt, state.cwd);
  if ((r.status !== 0 || r.error) && state.started) r = cli('claude', [...base, '--continue'], prompt, state.cwd);
  state.started = true;
...
export function ollamaChat(url, model, messages) {
  return new Promise((resolve) => {
    ...
    const req = http.request({ hostname: u.hostname, port: u.port || 80, path: u.pathname, method: 'POST', headers: { 'content-type': 'application/json', 'content-length': Buffer.byteLength(body) }, timeout: 10 * 60 * 1000 }, (res) => {
      let data = '';
...
export function newState(cwd, engine) {
  const sid = 'atlias-' + crypto.randomUUID();
  saveSessionMeta(sid, { host: 'atlias', cwd, source: 'startup', started: Date.now(), engine });
  return { sid, cwd, engine, started: false, history: [], messages: [] };
}
...
export async function repl(opts = {}) {
  ...
  for (;;) {
    ...
    if (t.startsWith('/engine ')) { const e = t.slice(8).trim(); if (detectEngines()[e]) { state.engine = e; out(d(`engine ${e}`)); } else out(c(`engine ${e} is not available here`)); continue; }
