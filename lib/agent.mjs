...
export function listChats(cwd) {
  let files = [];
  try { files = fs.readdirSync(CHATS()).filter((f) => f.endsWith('.chat.json')); } catch { return []; }
  return files.map((f) => readJson(path.join(CHATS(), f))).filter((c) => c && c.sid && (!cwd || path.resolve(c.cwd) === path.resolve(cwd))).sort((a, b) => (b.saved || 0) - (a.saved || 0));
}
...
export function loadChat(which, cwd) {
  ...
  if (which && which !== 'last' && which !== true) {
    ...
    if (!c) { const hits = listChats(null).filter((x) => x.sid.includes(String(which))); c = hits.length === 1 ? hits[0] : null; }
...
export function statusText(state) {
  const sent = loop.view(state.messages, config().agent.keepObservations);
...
export async function repl(opts = {}) {
  ...
  if (!state) state = newState(cwd, engine);
  if (state.engine !== engine) { state.engine = engine; state.started = false; state.messages = []; }
