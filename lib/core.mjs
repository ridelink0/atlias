...
export function writeLauncher(root = ROOT) {
  ...
  const body = [
    "#!/usr/bin/env node",
    "// Written by atlias. Host configs point here because this path never",
    "// changes, while the installed copy moves with every version.",
    "import { installedRoots } from " + JSON.stringify(pathToFileUrl(path.join(root, 'lib', 'core.mjs'))) + ";",
    "import path from 'node:path';",
    "import { pathToFileURL } from 'node:url';",
    "const roots = installedRoots();",
    "if (!roots.length) { process.stderr.write('atlias: no installed copy found\\n'); process.exit(1); }",
    "await import(pathToFileURL(path.join(roots[0], 'mcp', 'server.mjs')).href);",
    "",
  ].join("\n");
  writeText(LAUNCHER, body);
  return LAUNCHER;
}
function pathToFileUrl(p2) { return 'file:///' + String(p2).replace(/\\/g, '/').replace(/^\//, ''); }
