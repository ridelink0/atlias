...
export const DEFAULTS = {
  ...
  usage: { show: true },
  agent: { mode: 'both', engine: 'auto', ollamaUrl: 'http://127.0.0.1:11434', ollamaModel: 'gemma3:4b', ollamaNumCtx: 16384, ollamaNumPredict: 2048, ollamaKeepAlive: '', openaiUrl: 'https://api.openai.com/v1', openaiModel: '', nativeTools: true, maxToolRounds: 25, maxBadReplies: 3, keepObservations: 4, evictBlock: 4, outputBudget: 10000, testCommand: 'auto', permissions: 'workspace', sandbox: false },
};
...
export function ufsCopies(deps = {}) {
  ...
  const home = deps.home || HOME;
  const has = deps.exists || exists;
  const enabledMap = { ...((readJson(path.join(claudeDir, 'settings.json'), {}) || {}).enabledPlugins || {}), ...((readJson(path.join(claudeDir, 'settings.local.json'), {}) || {}).enabledPlugins || {}) };
  ...
  for (const [dir, loaded] of [[path.join(claudeDir, 'skills'), true], [path.join(home, '.agents', 'skills'), false]]) {
