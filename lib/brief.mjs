...
import { config, exists, readText, clip, claudeMemoryDir, readLines, saveSessionMeta, sessionMeta, HOST_LABEL, ROOT, VERSION, CLAUDE_DIR, readJson, findPython, log } from './core.mjs';
...
export function companions() {
  ...
  return {
    ufs: enabled.includes('ultimate-frontend-skills'),
...
export function build(payload, host) {
  ...
  if (g.exists) {
    const gods = graph.godNodes(cwd, cfg.graph.godNodes);
    ...
    const started = cfg.graph.autoBuild && graph.scheduleUpdate(cwd, { build: true });
    lines.push(`\n## Knowledge graph\nNone yet. ${started ? 'atlias started an AST-only build in the background (graphify update); it appears in the next brief.' : 'Build one with /graphify (semantic) or `graphify update .` (AST only).'}`);
  }
  ...
  const c = companions();
  const comp = [];
  comp.push(c.ufs ? 'ultimate-frontend-skills on (use /ultimate-frontend-skills for any page, screen or restyle)' : 'ultimate-frontend-skills off (install: claude plugin marketplace add ridelink0/ultimate-frontend-skills)');
  comp.push(c.graphify ? 'graphify ok' : 'graphify missing (pip install graphifyy)');
  if (c.usageLimits) comp.push('usage-limits on (its brief is the budget authority)');
  lines.push(`\n## Companions\n${comp.join('; ')}.`);
