...
export function proveTask(built, lang, { keep = false } = {}) {
  const spec = LANGS[lang];
  ...
  const write = (extraFiles = {}) => {
    ...
    ensureDir(dir);
    for (const [rel, body] of Object.entries({ ...built.task.files, ...extraFiles })) {
  ...
  const go = () => {
    let r = null;
    try { r = run(argv[0], argv.slice(1), { cwd: dir, timeout: built.task.timeoutMs || 120000 }); } catch (e) { return { status: null, out: String(e && e.message ? e.message : e) }; }
    return { status: r ? r.status : null, out: `${(r && r.stdout) || ''}${(r && r.stderr) || ''}`.trim().slice(-400) };
  };
  write();
  const before = go();
  if (before.status === 0) {
    if (!keep) { try { fs.rmSync(dir, { recursive: true, force: true }); } catch { /* best effort */ } }
    return { ok: false, why: 'the stub already passes, so the task would measure nothing', before, after: null };
  }
