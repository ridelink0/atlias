...
import { config, run, recordEvent, clip, isCodeFile, looksLikeVerification, programCommand, STATE_DIR } from './core.mjs';
...
export async function runTool(state, call, ask) {
  ...
  const writes = ['edit_file', 'write_file', 'apply_patch'].includes(call.tool);
...
export function view(messages, keep, step) {
  ...
  const oldest = cut > 0 ? obsIdx[cut] : -1;
  ...
  for (let round = 0; round < rounds; round++) {
    ...
    if (!calls.length) {
      ...
      if (check) {
        ...
        if (!/^refused/.test(out)) {
          state.messages.push({ role: 'user', content: `atlias ran the project's check after your edits (${check}):\n${out}\n\nIf it failed, find the cause, fix it and answer again. If it passed, answer with what changed and this result.`, _obs: { label: `check ${check}`, summary: clip(out.split('\n')[0], 100) } });
          state.obs++;
          continue;
        }
      }
