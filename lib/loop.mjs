...
const MORE = {
  shell: 'To see the rest, run it again narrowed down, or pipe it through a filter.',
  grep: 'To see the rest, narrow the pattern, or give grep a path.',
  outline: 'To see the rest, outline one file at a time.',
  list_dir: 'To see the rest, list_dir a subdirectory.',
  _assistant: 'The tool result below says what came of it.',
};
const MORE_DEFAULT = 'To see the rest, call it again with a narrower input.';
...
export const BAD_REPLY = { parse: 'a tool block that did not parse', edit: 'an edit that did not apply', truncated: 'a reply the provider cut off at its output limit' };
...
export function stallText(kinds) {
  const counts = [...new Set(kinds)].map((k) => `${kinds.filter((x) => x === k).length} of ${BAD_REPLY[k] || k}`);
  return `atlias stopped: ${kinds.length} model replies in a row produced nothing usable (${counts.join('; ')}), so the task is not finished. This is not the round budget running out - the model never sent a usable action. Try a stronger model, or raise agent.maxBadReplies with atlias settings if it needs more tries.`;
}
