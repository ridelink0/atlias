...
export function harnessStamp(root = ROOT, files = STAMP_FILES) {
  ...
  let r = null;
  try { r = run('git', ['log', '-1', '--format=%h', '--', ...files], { cwd: root, timeout: 5000 }); } catch { r = null; }
  const sha = r && r.status === 0 ? String(r.stdout || '').trim().split('\n')[0].trim() : '';
  if (!sha) return { version, sha: '', dirty: false, dirtyFiles: [], text: `${name}, unstamped` };
  let dirtyFiles = [];
  try {
    const d = run('git', ['status', '--porcelain', '--', ...files], { cwd: root, timeout: 5000 });
