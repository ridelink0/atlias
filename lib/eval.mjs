...
export async function runSuite(tasks, opts = {}) {
  ...
  return {
    ...
    budgetRange: (() => {
      const own = results.map((r) => Number(r.budget) || 0).filter((n) => n > 0);
      return own.length ? [Math.min(...own), Math.max(...own)] : null;
    })(),
  };
}
...
export function format(report) {
  ...
  else if (report.budgetRange) bits.push(report.budgetRange[0] === report.budgetRange[1] ? `${report.budgetRange[0]} rounds per task` : `${report.budgetRange[0]}-${report.budgetRange[1]} rounds per task`);
