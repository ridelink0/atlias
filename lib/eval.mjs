...
export async function runTask(task, { chat, state, stamp = String(Date.now()), keep = false } = {}) {
  ...
  // totals again.
  const st = { ...(state || {}), cwd: dir, messages: [], editTries: 0, editFails: 0, editWhy: {} };
  const t0 = Date.now();
...
export function bootstrapDiff(pairs, rounds = 2000, rng = Math.random) {
  ...
  const diffs = pairs.map((x) => (x.b ? 1 : 0) - (x.a ? 1 : 0));
  const means = [];
...
// comparison.
export function compare(a, b, { rounds = 2000, rng = Math.random } = {}) {
  ...
  return {
    ...
    verdict: test.n === 0 ? 'nothing changed on any task'
      : test.p > 0.05 ? 'inside the noise: a coin lands this way about as often'
...
export function formatCompare(c, aName = 'A', bName = 'B') {
  ...
  lines.push(`McNemar exact p = ${c.p.toFixed(3)} on ${c.changed} disagreement(s): ${c.verdict}.`);
  lines.push(`Paired bootstrap on the pass rate: ${(c.boot.mean * 100).toFixed(1)} points, 95 per cent interval ${(c.boot.lo * 100).toFixed(1)} to ${(c.boot.hi * 100).toFixed(1)}.`);
