...
**mini-swe-agent 2.4.6 on the same 27 tasks: also 0 of 27, in 1281 s** (`poly-mini.json`), same Ollama model, 18 steps per task, a fixed 16,384 window and `num_predict` 2048, no Docker. `atlias compare poly-B.json poly-mini.json`: 0 gained, 0 lost, p = 1.000, "nothing changed on any task". Neither harness finishes a single polyglot task with this model, so this corpus cannot rank them.

What differs, and what that is worth:

- **Context moved: atlias 725k characters, mini-swe-agent 431k**, on the same tasks and the same budget. atlias spent about 1.7 times the characters to reach the same zero. mini-swe-agent counts its messages the same way (all message content), but its step and atlias's tool round are close, not identical.
- **Wall time: 1551 s against 1281 s**, both with the model kept loaded throughout. That is 21 per cent more for atlias, and the machine was not otherwise idle for either, so it is a direction, not a measurement.
- **Partial credit went the other way.** Read with atlias's own `caseCounts` over the 300-character test tail the driver keeps, mini-swe-agent's workspaces pass **78 of 385** test cases, with 10 tasks above zero (pig-latin 20 of 22, bowling 21 of 31, variable-length-quantity 14 of 26); atlias's pass **41 of 429**, with 8 above zero. The denominators differ because a solution that does not import collects no tests, so these are not a paired measurement, but on the one axis where either harness shows progress on this corpus, the simpler one is ahead. The comparator prints it as "not recorded" for mini-swe-agent because the driver does not save case counts; the figure here is my read of its saved output.
...
Reading, stated plainly: after the window fix atlias is not better than a 100-line bash-only agent on this corpus and this model, and it is more expensive. The biggest measured difference in atlias's own counters is the JSON edit call that will not parse (25 of 60 failures), and mini-swe-agent has no such call. That is the evidence item 4 and 5 were waiting for, and it points at the "Later" item for a non-JSON edit channel too.
