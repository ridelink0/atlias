...
export default async function exitSuites({ suite, asyncSuite, check, core, agentMod, TMP, fs, path }) {
  ...
  suite('exit reason expert', 'every way out of a run names itself', () => {
    ...
    check('the text says how many of what, and that it is not the round budget', /3 model replies in a row/.test(text) && /2 of a tool block that did not parse/.test(text) && /1 of an edit that did not apply/.test(text) && /not the round budget/.test(text) && /agent\.maxBadReplies/.test(text), { happened: text, why: 'The two stops look identical from outside - no answer came back - and the whole value of counting them apart is lost if the message does not say which happened.', fix: 'Check stallText and BAD_REPLY.' });
    check('every kind of bad reply has words of its own', Object.keys(loop.BAD_REPLY).length === 3 && Object.values(loop.BAD_REPLY).every((v) => v.length > 10), { happened: JSON.stringify(loop.BAD_REPLY), why: 'A kind with no wording would print its own variable name at the user.', fix: 'Keep BAD_REPLY in step with what runLoop pushes onto bad.' });
  ...
  await asyncSuite('stall expert', 'replies that produce nothing are counted apart from the rounds', async () => {
    ...
    check('three edits in a row that do not apply stop the run too', st3.stop && st3.stop.reason === 'malformed-output' && /an edit that did not apply/.test(reply3) && st3.stop.detail.includes('did not apply'), { happened: JSON.stringify(st3.stop) + ' | ' + reply3.slice(0, 140), why: 'A weak model that cannot match old_string will send the same failing edit until the rounds run out, and the report blamed the budget.', fix: 'Check deadEdits in runLoop; a failed edit is one that did not move state.editedAt.' });
